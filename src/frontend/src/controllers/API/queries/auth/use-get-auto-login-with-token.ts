...
export interface AutoLoginWithTokenResponse {
...
export const useGetAutoLoginWithToken: useQueryFunctionType<undefined, undefined> = (
  options,
) => {
  ...
  async function getAutoLoginWithTokenFn(): Promise<null> {
    try {
      // Get the iframe token
      const iframeToken = getIframeToken();

      if (!iframeToken) {
        console.warn("No iframe token available for auto login with token");
        setAutoLogin(false);
        if (!isLoginPage) {
      ...
      // Call the new backend endpoint with the token
      const response = await api.post<Users>(`${getURL("AUTO_LOGIN_WITH_TOKEN")}`, {
        token: iframeToken,
      });
  ...
  const queryResult = query(["useGetAutoLoginWithToken"], getAutoLoginWithTokenFn, {
